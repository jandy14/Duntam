...
void GameManager::CreateDebugMap()		//디버그 용 맵 생성
{
	//여긴 우째야 할지...
	for (int y = 0; y < 9; y++) 
		for (int x = 0; x < 9; x++)
	...
	//for (int i = 0; i < 15; i++)
		//map[4][4]->objectList.push_front(new EnemyC(i, i));
	map[4][4]->objectList.push_front(new EnemyC(5, 5));
	map[4][4]->objectList.push_front(new EnemyD(10, 10));
	map[4][4]->objectList.push_front(new EnemyE(20, 10));
}
